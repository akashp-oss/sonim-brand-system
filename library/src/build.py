...
CSS = r"""
:root{--red:#CF102D;--black:#000;--gray:#E5ECEE;--white:#fff;--line:#E5ECEE;--ui:Arial,"Liberation Sans",Helvetica,sans-serif;--top:60px;--side:264px}
...
/* ---------- app bar ---------- */
.topbar{position:fixed;top:0;left:0;right:0;height:var(--top);background:#fff;border-bottom:1px solid var(--line);display:flex;align-items:center;gap:8px;padding:0 16px 0 0;z-index:30}
...
.topbar .brand span{font-size:12px;border-left:1px solid var(--line);padding-left:14px;white-space:nowrap}
.btn{font:700 12px/1 var(--ui);color:#000;background:#fff;border:1px solid var(--line);border-radius:0;height:34px;padding:0 12px;cursor:pointer;white-space:nowrap}
...
/* ---------- sidebar ---------- */
.sidebar{position:fixed;top:var(--top);bottom:0;left:0;width:var(--side);border-right:1px solid var(--line);background:#fff;overflow:auto;padding:24px 0 32px;z-index:20}
...
.scrim{display:none}
/* ---------- canvas ---------- */
.canvas{margin:var(--top) 0 0 var(--side);padding:40px 48px 120px}
.lib-section{margin-bottom:88px}
.sec-head{display:flex;gap:40px;align-items:flex-start;margin:0 0 32px -48px}
...
.frames{display:flex;flex-wrap:wrap;gap:48px 40px;align-items:flex-start}
...
.ab-wrap{position:relative;z-index:0;overflow:hidden;outline:1px solid var(--line)}
...
.tool{font:700 11px/1 var(--ui);height:28px;padding:0 10px;border:1px solid #000;background:#fff;color:#000;cursor:pointer;display:flex;align-items:center;gap:6px;white-space:nowrap}
.tool:hover{background:#000;color:#fff}
.tool.icon{padding:0 7px}
.tool.done{background:var(--red);border-color:var(--red);color:#fff}
...
<header class="topbar" data-name="App bar">
  ...
  <div class="brand">{LOGO_UI}<b>Brand template library</b><span>{total} templates · one system</span></div>
